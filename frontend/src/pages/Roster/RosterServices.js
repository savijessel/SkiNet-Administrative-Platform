...
export async function getCalendarData(
  ...
  try {
    if (Updater) {
      ...
      let sendString =
        low_year + "-" + high_year + "-" + low_month + "-" + high_month;
      await axios
        .get("/getEvents/" + sendString)
        .then((response) => {
      ...
      if (dragDropEnable === "first") setDragDropEnable("second");
      else if (dragDropEnable === "second") setDragDropEnable("third");
    }
  } catch (error) {
    console.log(error);
  }
...
export const selectShiftHandler = async (
  clickInfo,
  setCurrentShift,
  currentShift,
  dragDropEnable,
...
export const linkShiftHandler = async (
  ...
  try {
    ...
    //Setting on and off of pop up
    await axios
      .get("/getShiftInfo/" + linkId.event.id)
      .then((response) => {
        //correct response
        if (response.status === 200) {
          setShiftInfo({
    ...
    //Getting the action log
    axios
      .get("/getActionLogInfo/" + linkId.event.id)
      .then((response) => {
        //correct response
        if (response.status === 200) {
    ...
    //Getting the Event Log Users
    await axios
      .get("/getEventLogInfo/" + linkId.event.id)
      .then((response) => {
        //correct response
        if (response.status === 200) {
          for (let i = 0; i < response.data.length; i++) {
    ...
    if (dragDropEnable === "third") {
      ...
      for (let i = 0; i < shifts.length; i++) {
        if (shifts[i].id === e.event.id) {
          ...
          let startString =
            newTempStart.getFullYear() +
            "-" +
            (month < 10 ? "0" + month : month) +
            "-" +
            (newTempStart.getDate() < 10
              ? "0" + newTempStart.getDate()
              : newTempStart.getDate());

          //End String
          month = newTempEnd.getMonth() + 1;
          let endString =
            newTempEnd.getFullYear() +
            "-" +
            (month < 10 ? "0" + month : month) +
            "-" +
            (newTempEnd.getDate() < 10
              ? "0" + newTempEnd.getDate()
              : newTempEnd.getDate());
      ...
      for (let i = 0; i < newShiftInfo.length; i++) {
        ...
        let good = false;
        await axios.put("/updateDragDrop", article).then((response) => {
          //if error from database
